...
def add_pop_info_to_impact_data(iso3, global_ids_mun, global_pop, impact_global):
    ...
    # Level 1
    population_sum = df_merge_country.groupby('GID_1')['population'].sum().reset_index()
    gid_0_data = df_merge_country.groupby('GID_1').first().reset_index()[['GID_1', 'GID_0']]
    df_merge_gid1 = pd.merge(gid_0_data, population_sum, on='GID_1')

    # Level 2
    population_sum = df_merge_country.groupby('GID_2')['population'].sum().reset_index()
    gid_0_data = df_merge_country.groupby('GID_2').first().reset_index()[['GID_2', 'GID_1', 'GID_0']]
    df_merge_gid2 = pd.merge(gid_0_data, population_sum, on='GID_2')

    # Add information as new columns
    impact_plus = impact_country.merge(df_merge_gid1[['population', 'GID_1']], 
                                        on='GID_1', how='left').rename({
                                            'population':'population_gid1'}, axis=1)

    impact_plus = impact_plus.merge(df_merge_gid2[['population', 'GID_2']], 
                                        on='GID_2', how='left').rename({
                                            'population':'population_gid2'}, axis=1)
...
def impact_to_grid(impact_merged_adm):
    ...
    for typhoon_id in df_events['DisNo.'].unique():
        ...
        df_event_dmg_with_pop = df_event[(df_event['population'] > 1) & (df_event['Total Affected'] !=0)].copy()
        ...
        perc_dmg = 100 * df_event_dmg_with_pop['Total Affected'].unique() /  df_event_dmg_with_pop['population'].sum()
        ...
        df_event_dmg_with_pop.loc[:, 'perc_affected_pop_grid_region'] = 100 * df_event_dmg_with_pop['Total Affected'] / TOTAL_POP_REG
        ...
        df_event_dmg_with_pop.loc[:, 'perc_affected_pop_grid'] = 100 * df_event_dmg_with_pop['population'] * df_event_dmg_with_pop['Total Affected'] / TOTAL_POP_REG ** 2
        df_event = df_event.merge(df_event_dmg_with_pop, how='left').fillna(0)
        impact_data_grid_no_weather = pd.concat([impact_data_grid_no_weather, df_event])
...
def process_impact_level(level, impact_plus, pop_grid):
    try:
        ...
        impact_merged = impact_level.merge(pop_grid, on=['GID_1', 'GID_2'])[
            ['DisNo.', 'sid', 'level', 'population', 'id', 'GID_1', 'GID_2', 'Total Affected']
        ].drop_duplicates()
        return impact_to_grid(impact_merged)
    except:
        return pd.DataFrame()
    
def create_impact_grid_dataset(iso3, global_ids_mun, global_pop, impact_global):
    # Population and impact data
    impact_plus, pop_grid = add_pop_info_to_impact_data(iso3, global_ids_mun, global_pop, impact_global)
    ...
    # Concatenate results
    impact_data_grid = pd.concat([impact_data_grid_adm1, impact_data_grid_adm2])
...
def iterate_grid_impact(iso3_list, global_ids_mun, global_pop, impact_global):
    ...
    for iso3 in iso3_list:
        impact_data_grid = create_impact_grid_dataset(iso3, global_ids_mun, global_pop, impact_global)
        impact_data_grid_global = pd.concat([impact_data_grid_global, impact_data_grid])
    # Reset index
...
if __name__ == "__main__":
    ...
    impact_data_grid_global = iterate_grid_impact(iso3_list=iso3_list, 
                                                  global_ids_mun=global_ids_mun, 
                                                  global_pop=global_pop, 
                                                  impact_global=impact_global)
    # Upload to blob in chunks
    blob.upload_in_chunks(dataframe=impact_data_grid_global,
                          chunk_size=100000,
                          blob=blob,
