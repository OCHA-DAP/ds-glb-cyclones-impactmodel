...
features_all = features + ['perc_affected_pop_grid_region', 'DisNo.' ,'sid', 'id' ,'iso3', 'typhoon']
...
df_global = pd.read_csv('/home/fmoss/GLOBAL MODEL/ds-glb-cyclones-impactmodel/src_global/datasources/model_input_data.csv')
df_global = df_global.drop_duplicates(subset=['sid', 'iso3', 'id', 'perc_affected_pop_grid_region'])

# Drop particular events
df_global['typhoon'] = df_global.typhoon_name + df_global.typhoon_year.astype('str')
weird_events = pd.read_csv('/home/fmoss/GLOBAL MODEL/ds-glb-cyclones-impactmodel/local_files/weird_cases.csv')
weird_events['iso3'] = weird_events['GID_0']

# Filter out the events
df_global_filtered = df_global[~df_global.typhoon.isin(weird_events.typhoon)].reset_index(drop=True)
...
def xgb_model_LOOCV(df, features, following='HTI', 
                    weight=2, target_name='perc_affected_pop_grid_grid', validation_events=None):
    ...
    def custom_track_loss(y_true, y_pred, track_distance):
        ...
        gamma = 0.1
        weights = np.exp(gamma * np.array(track_distance))  # Convert track_distance to numpy array if it's a Series
    ...
    for sid, iso3, DisNo in zip(aux['sid'], aux['iso3'], aux['DisNo.']):
        ...
        df_test = df[
            (df["sid"] == sid) & 
            (df["iso3"] == iso3) &
            (df["DisNo."] == DisNo)] # Test set: HTI event
        df_train = df[
            (df["sid"] != sid) & 
            (df["iso3"] != iso3) &
            (df["DisNo."] != DisNo)] # Train set: everything else
        ...
        # Class weight
        weights = np.where(
            df_train['iso3'] == following, weight, 1) 
        ...
        xgb = XGBRegressor(
            ...
            objective=lambda y_true, y_pred: custom_track_loss(y_true, y_pred, X_train["track_distance"].values),
            reg_alpha=0,
            reg_lambda=1,
            scale_pos_weight=1,
            seed=None,
...
def xgb_model_LOOCV_boot(df, features, following='HTI', weight=2, 
                         cycle=10, frac=0.8, 
                         shuffle=True, bootstrapping=True,
...
if __name__ == "__main__":
    ...
    if boot:
        ...
        np.savez(f'{country_of_interest}_xgb_weather_constraints_boot.npz', 
                 array1=y_test, array2=y_pred)
    else:
        ...
        # Save them to a .npz file (which stores multiple arrays)
        np.savez(f'{country_of_interest}_xgb_no_boot_subset_validation_custom_objective_track_distance.npz', 
                 array1=y_test, array2=y_pred)