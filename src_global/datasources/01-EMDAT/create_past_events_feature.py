...
def count_events_in_last_5_years(df, year_col="Start Year", month_col="Start Month", country_col="GID_0", event_col="DisNo."):
    """
    ...
    df['Event Date'] = df.apply(lambda row: to_datetime(row[year_col], row[month_col]), axis=1)
    ...
    def count_events_for_row(row):
        ...
        last_5_years_data = country_data[(country_data['Event Date'] >= five_years_ago) & (country_data['Event Date'] <= event_date)]
    ...
    return df[[event_col, country_col, year_col, month_col, "N_events_5_years"]]
...
if __main__ == "__main__":
    ...
    # Save the output
    last_5_years.to_csv("/data/big/fmoss/data/EMDAT/impact_in_last_5_years_data.csv", index=False)
