...
def get_wind_data(PROJECT_PREFIX="global_model"):
    ...
    filenames = [f"wind_data_part_{i}.csv" for i in range(1, 184)]
    dataframes = []
...
def get_rain_data(PROJECT_PREFIX="global_model"):
    # Load data (is in chunks)
    filenames = [f"rainfall_data_part_{i}.csv" for i in range(1, 174)]
    dataframes = []
    for filename in filenames:
...
def merge_weather_impact_population(
    wind_data,
    rain_data,
    impact_data,
    population_data,
    ...
    if weather_constraints:
        ...
        impact_weather_merged = weather_data_merged.merge(
            impact_data, on=["sid", "iso3"], how="inner"
        )

    # Step 3: Merge with population data
    input_data_merged = impact_weather_merged.merge(
        population_data, on=["iso3", "id"], how="inner"
    )

    # Step 4: Drop unnecessary columns and remove duplicates
    input_data_merged = (
        input_data_merged.drop(["GID_0", "GID_1", "GID_2"], axis=1)
        .drop_duplicates()
...
def save_input_data_to_blob(weather_constraints=False):
    ...
    blob_name_template = (
        "model_input_data_weather_constraints"
        if weather_constraints
        else "model_input_data"
    )
...
if __name__ == "__main__":
    ...
    user_input = input(
        "Would you like to enable weather constraints? (True/False): "
    )
    ...
    flood_risk = pd.read_csv(
        "/home/fmoss/GLOBAL MODEL/data/FloodRisk/global_flood_risk_index.csv"
    )
    landslide_risk = pd.read_csv(
        "/home/fmoss/GLOBAL MODEL/data/landslides/global_landslide_risk_index.csv"
    )
    static_risks = flood_risk.merge(
        landslide_risk[["id", "iso3", "landslide_risk_sum"]]
    )
    vulnerability_meta = pd.read_csv(
        "/home/fmoss/GLOBAL MODEL/data/vulnerability_from_metadata/metadata_vulnerability_feature.csv"
    )
