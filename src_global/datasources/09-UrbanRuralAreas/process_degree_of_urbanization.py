...
def calculate_urban_rural_water(grid, raster, out_dir, iso3, nodata_value=128):
    ...
    smod_grid_vals["urban"] = (
        smod_grid_vals.get(21, 0) + smod_grid_vals.get(22, 0) + smod_grid_vals.get(23, 0)
    ) / smod_grid_vals.sum(axis=1)

    smod_grid_vals["rural"] = (
        smod_grid_vals.get(11, 0) + smod_grid_vals.get(12, 0) + smod_grid_vals.get(13, 0)
    ) / smod_grid_vals.sum(axis=1)

    smod_grid_vals["water"] = (
        smod_grid_vals.get(10, 0)
    ) / smod_grid_vals.sum(axis=1)
...
def process_country(iso3, grid_transformed, src_wgs84, out_dir, nodata_value=128):
    ...
    calculate_urban_rural_water(country_grid, src_wgs84, out_dir, iso3, nodata_value)
...
def process_all_countries(grid_transformed, src_wgs84, out_dir, iso3_list, nodata_value=128):
    ...
    for iso3 in iso3_list:
        process_country(iso3, grid_transformed, src_wgs84, out_dir, nodata_value)
...
if __name__ == "__main__":
    ...
    # Load grid cells
    grid_training = gpd.read_file("/home/fmoss/GLOBAL MODEL/data/GRID/global_0.1_degree_grid_land_overlap.gpkg")
