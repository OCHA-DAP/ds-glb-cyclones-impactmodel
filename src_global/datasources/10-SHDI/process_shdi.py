...
def interpolate_missing_shdi(admin_shdi_data):
    ...
    valid_countries = df.groupby("country_id")["shdi"].transform(lambda x: x.notna().any())
    ...
    def apply_idw(group):
        ...
        group.loc[group["shdi"].isna(), "shdi"] = idw(distances, neighbor_values)
        
        return group

    df_interpolated = df_filtered.groupby("country_id", group_keys=False).apply(apply_idw)
    return df_interpolated


if __name__ == "__main__":
    # Load shdi 
    shdi = pd.read_csv(f"{data_path}/{file}", low_memory=False)[['iso_code', 'year', 'GDLCODE', 'level', 'shdi']]
    ...
    gadm_merged.to_file("/data/big/fmoss/data/SHDI/GADM_adm1_SHDI.gpkg", driver='GPKG')
    ...
    grid_shdi = grid_cells.merge(
        gadm_interpolated[['GID_0', 'GID_1', 'shdi']], on=['GID_0', 'GID_1'], how='left'
        ).sort_values('GID_0', ignore_index=True)
    os.mkdir("/data/big/fmoss/data/SHDI/grid_data/")
    grid_shdi[
        ['GID_0', 'GID_1', 'id', 'shdi']
        ].to_csv("/data/big/fmoss/data/SHDI/grid_data/shdi_global_grid_index.csv", index=False)
