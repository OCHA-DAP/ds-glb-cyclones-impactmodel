...
def download_tif_files(url, download_folder='./tif_files_floods', adjusted=False):
    """
    ...
    if adjusted:
        tif_files = [link.get('href') for link in links if link.get('href').endswith('_depth_reclass.tif')]
    else:
        tif_files = [link.get('href') for link in links if link.get('href').endswith('_depth.tif')]
...
if __name__ == "__main__":
    ...
    tif_paths = download_tif_files(url, download_folder=data_folder, adjusted=True)