...
def get_mosaic_raster_paths(row, data_path):
    ...
    mosaic_raster_paths = [tile_path for tile_path in results if tile_path is not None]

    return mosaic_raster_paths

def calculate_zonal_stats(grid, raster_array, stats, nodata, affine, stat_type):
    """
...
def process_single_raster(grid, raster_path):
    ...
    def process_slope_task(raster_path):
        ...
        with rasterio.open(slope_path) as slope_rast:
            ...
            return process_slope(grid, slope_array, slope_rast.transform), slope_path

    def process_ruggedness_task(raster_path):
        tri_path = tempfile.NamedTemporaryFile(suffix=".tif", delete=False).name
        subprocess.run(
            [
                "gdaldem",
                "TRI",
        ...
        with rasterio.open(tri_path) as tri_rast:
            ...
            return process_ruggedness(grid, tri_array, tri_rast.transform), tri_path
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            ...
            altitude_future = executor.submit(process_altitude_task, raster_path)
            slope_future = executor.submit(process_slope_task, raster_path)
            ruggedness_future = executor.submit(process_ruggedness_task, raster_path)
...
def main_processing_pipeline(grid, mosaic_raster_paths, num_workers=4):
    ...
    altitude_df = pd.concat([res[0].dropna() for res in results], ignore_index=True)
    slope_df = pd.concat([res[1].dropna() for res in results], ignore_index=True)
    ruggedness_df = pd.concat([res[2].dropna() for res in results], ignore_index=True)
...
def merge_terrain_data(grid, altitude_df, slope_df, ruggedness_df):
    ...
    df_rugged = ruggedness_df.rename({"mean": "mean_rug"}, axis=1) #.fillna(0)
...
def get_coast_features(shp, grid):
    ...
    grid_coast = grid[["id"]].merge(
        grid_line_gdf, on=["id"], how="left"
    )
...
def data_extraction(iso, grid, shp, country_tiles, out_path, data_path):
    try:
        ...
        if os.path.exists(output_path):
            ...
            # Get Altitude, Slope and Ruggedness data
            mosaic_raster_paths = country_tiles[country_tiles.iso3 == iso].apply(
                    lambda row: get_mosaic_raster_paths(row=row, data_path=data_path), axis=1
                ).iloc[0]
            ...
            if len(mosaic_raster_paths) == 0:
                ...
                df_srtm = merge_srtm_features(df_coast, df_terrain, spatial_interpolation=False)
                df_srtm['iso3'] = iso

            else:
                altitude_df, slope_df, ruggedness_df = main_processing_pipeline(
                        grid_country, 
                        mosaic_raster_paths, 
                        num_workers=4
                        )
                # Terrain features all together
                df_terrain = merge_terrain_data(grid_country, altitude_df, slope_df, ruggedness_df)
                # Get Coastline data
                df_coast = get_coast_features(shp_country, grid_country)
...
def iterate_srtm_data_extracting(grid, shp, out_path, data_path):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        ...
        future_to_iso = {executor.submit(data_extraction, iso, grid, shp, country_tiles, out_path, data_path): iso for iso in iso_list}
