...
def download_tiles(iso3, global_grid, base_url, PROJECT_PREFIX='global_model', transform_geometry=False):
    ...
    if transform_geometry:
        ...
        country_adm2["geometry"] = country_adm2["geometry"].apply(adjust_longitude)
            ...
            for fileName in fileNames:
                if fileName.endswith("tif"):
                    ...
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".tif") as temp_file:
                        ...
                        check = len(blob.list_container_blobs(name_starts_with=blob_name))
                        if check == 1:
                            print(f"{fileName} tile already in the {iso3} folder")
                            pass
                        else:
                            # print(f"Uploading {fileName} to {iso3} folder")
                            blob.upload_tif_to_blob(file_path=temp_file_path,
                                                    blob_name=blob_name)
...
if __name__ == "__main__":
    ...
    global_shp = blob.load_gpkg("global_model/SHP/global_shapefile_GID_adm2.gpkg")
    # Download data
    download_process(global_shp)
