...
def rasterize_coastline(coastline, output_tif, resolution=0.01):
    ...
    shapes = ((geom, 1) for geom in coastline.geometry)  # Assign a value of 1 to lines
...
def create_coastline_tif(shp, out_dir):
    ...
    coastline = gpd.GeoDataFrame(geometry=coastline.explode(index_parts=False), crs=shp.crs)
...
def get_coast_features_from_raster(grid, raster_data):
    ...
    grid.loc[:, "coast_pixels"] = [s["sum"] if s["sum"] is not None else 0 for s in stats]

    # Convert coastline pixels to meters
    grid.loc[:, "coast_length"] = grid["coast_pixels"] * raster_data["pixel_size"]
...
def parallel_process_countries(grid, raster_data, out_dir, max_workers=4):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        futures = [
            executor.submit(process_country, grid, country, raster_data, out_dir)
